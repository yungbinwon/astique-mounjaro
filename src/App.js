...
function StockView({showToast}) {
  ...
  async function save() {
    const ok=await fbSetStock(draft);
    if(!ok) return showToast("บันทึกไม่สำเร็จ");
    setStock(draft);
    setEditing(false);
    showToast("✓ อัพเดท Stock แล้ว");
  }

  function startEdit() {
    const d={};
    STOCK_DOSES.forEach(dose=>{ d[dose]=stock[dose]||0; });
    setDraft(d);
    setEditing(true);
  }
  ...
  return <div style={{padding:"18px 14px"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
      {STOCK_DOSES.map(dose=>{
        ...
        const low=qty<=2;
        const out=qty===0;
        return <div key={dose} style={{...cardSt,marginBottom:0,border:`1.5px solid ${out?C.alert:low?"#C4993E":C.blush}`,background:out?"rgba(201,59,59,0.04)":low?"rgba(196,153,62,0.04)":C.card}}>
          <div style={{fontSize:12,color:C.muted,marginBottom:4,fontWeight:500}}>Mounjaro</div>
          <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:22,fontWeight:600,color:C.deep,marginBottom:6}}>{dose}</div>
          {editing
            ?<div style={{display:"flex",alignItems:"center",gap:8}}>
              <button onClick={()=>setDraft(d=>({...d,[dose]:Math.max(0,(d[dose]||0)-1)}))} style={{width:32,height:32,borderRadius:8,border:`1px solid ${C.blush}`,background:"transparent",fontSize:18,cursor:"pointer",color:C.deep}}>−</button>
              <input type="number" inputMode="numeric" style={{...inputSt,textAlign:"center",padding:"6px",width:60,fontSize:18,fontWeight:600}} value={draft[dose]||0} onChange={e=>setDraft(d=>({...d,[dose]:Math.max(0,parseInt(e.target.value)||0)}))}/>
              <button onClick={()=>setDraft(d=>({...d,[dose]:(d[dose]||0)+1}))} style={{width:32,height:32,borderRadius:8,border:`1px solid ${C.blush}`,background:"transparent",fontSize:18,cursor:"pointer",color:C.deep}}>+</button>
            </div>
            :<div style={{display:"flex",alignItems:"baseline",gap:6}}>
              <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:32,fontWeight:600,color:out?C.alert:low?C.gold:C.deep}}>{qty}</span>
    ...
    {!editing&&<div style={{marginTop:14,...cardSt,background:"rgba(196,153,62,0.06)",border:`1px solid ${C.blush}`}}>
      <div style={{fontSize:12,color:C.muted,marginBottom:6}}>📋 สรุป Stock</div>
      <div style={{fontSize:13,color:C.deep,lineHeight:1.8}}>
        {STOCK_DOSES.map(dose=>{
          const qty=stock[dose]||0;
          return <div key={dose} style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <span>Mounjaro {dose}</span>
            <span style={{fontWeight:600,color:qty===0?C.alert:qty<=2?C.gold:C.sage}}>{qty} หลอด {qty===0?"🔴":qty<=2?"🟡":"🟢"}</span>
          </div>;
        })}
      </div>
    </div>}
...
// ── OrderView ─────────────────────────────────────────────
const ORDER_TYPES = ["มาฉีดที่คลินิก","จัดส่งทางไปรษณีย์"];
const ORDER_STATUS = {
  pending: {label:"รอดำเนินการ", color:"#C4993E", bg:"rgba(196,153,62,0.1)", emoji:"🟡"},
...
function OrderForm({onSave,saving,showToast,onCancel}) {
  ...
  function submit() {
    if(!name.trim()) return showToast("กรุณากรอกชื่อคนไข้");
    if(!dose) return showToast("กรุณาเลือก dose");
    if(!type) return showToast("กรุณาเลือกประเภท");
    if(type==="จัดส่งทางไปรษณีย์"&&!address.trim()) return showToast("กรุณากรอกที่อยู่จัดส่ง");
    onSave({name:name.trim(),dose:dose==="อื่นๆ"?doseOther:dose,type,address,amount,note});
  }

  return <div style={{padding:"18px 14px"}}>
    <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
      <button onClick={onCancel} style={secBtnSt}>← กลับ</button>
      <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,fontWeight:600,color:C.deep}}>เพิ่ม Order ใหม่</div>
    </div>
    <div style={cardSt}>
      <CardTitle>ข้อมูลออเดอร์</CardTitle>
      <div style={{marginBottom:12}}>
        <label style={labelSt}>ชื่อ-นามสกุลคนไข้</label>
        <input style={inputSt} placeholder="เช่น สมหญิง รักสุขภาพ" value={name} onChange={e=>setName(e.target.value)}/>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
        <div>
          <label style={labelSt}>ขนาดยา</label>
        ...
        <div>
          <label style={labelSt}>ยอดโอน (บาท)</label>
          <input type="number" inputMode="numeric" style={inputSt} placeholder="0" value={amount} onChange={e=>setAmount(e.target.value)}/>
        </div>
      </div>
      {dose==="อื่นๆ"&&<div style={{marginBottom:12}}><label style={labelSt}>ระบุขนาดยา</label><input style={inputSt} placeholder="เช่น 20 mg" value={doseOther} onChange={e=>setDoseOther(e.target.value)}/></div>}
      <div style={{marginBottom:12}}>
        <label style={labelSt}>ประเภท</label>
        <div style={{display:"flex",gap:8}}>
          {ORDER_TYPES.map(t=>{const on=type===t;return <button key={t} onClick={()=>setType(t)} style={{flex:1,padding:"10px 8px",borderRadius:10,fontSize:12,border:`1.5px solid ${on?C.rose:C.blush}`,background:on?C.rose:"transparent",color:on?"#fff":C.muted,fontFamily:"'DM Sans',sans-serif",cursor:"pointer",fontWeight:on?600:400}}>{t==="มาฉีดที่คลินิก"?"🏥 มาฉีดที่คลินิก":"📦 จัดส่งทางไปรษณีย์"}</button>;})}
        </div>
      </div>
      {type==="จัดส่งทางไปรษณีย์"&&<div style={{marginBottom:12}}><label style={labelSt}>ที่อยู่จัดส่ง</label><textarea style={{...inputSt,height:80,resize:"none",lineHeight:1.5}} placeholder="บ้านเลขที่ ถนน ตำบล อำเภอ จังหวัด รหัสไปรษณีย์" value={address} onChange={e=>setAddress(e.target.value)}/></div>}
      <div style={{marginBottom:4}}>
        <label style={labelSt}>หมายเหตุ</label>
        <input style={inputSt} placeholder="เช่น แพ้ยาเดิม นัดวันศุกร์..." value={note} onChange={e=>setNote(e.target.value)}/>
      </div>
      <button style={{...btnSt,opacity:saving?0.7:1}} onClick={submit} disabled={saving}>{saving?"กำลังบันทึก...":"✦ บันทึก Order"}</button>
    </div>
  </div>;
}
...
function OrderView({showToast}) {
  ...
  const [orderTab,setOrderTab]=useState("orders"); // orders | stock
  ...
  async function handleSave(order) {
    setSaving(true);
    const ok=await fbAddOrder(order);
    setSaving(false);
    if(!ok) return showToast("บันทึกไม่สำเร็จ");
    showToast("✓ เพิ่ม Order สำเร็จ");
    setShowForm(false);
    load();
  }

  async function updateStatus(id,status,orderDose) {
    await fbUpdateOrder(id,{status,updatedAt:new Date().toISOString()});
    // auto-deduct stock when marking as done
    if(status==="done"&&orderDose) {
      const matchDose=STOCK_DOSES.find(d=>orderDose.includes(d.replace(" mg",""))||orderDose===d);
      if(matchDose) {
        const stock=await fbGetStock();
        const cur=stock[matchDose]||0;
        if(cur>0) {
          await fbSetStock({...stock,[matchDose]:cur-1});
          showToast(`✓ เสร็จสิ้น — ตัด ${matchDose} 1 หลอด`);
        } else {
          showToast("⚠️ Stock หมด! กรุณาเติม Stock");
        }
  ...
  if(showForm) return <OrderForm onSave={handleSave} saving={saving} showToast={showToast} onCancel={()=>setShowForm(false)}/>;

  const pending=orders.filter(o=>o.status==="pending");
  const active=orders.filter(o=>o.status==="clinic"||o.status==="shipping");
  const done=orders.filter(o=>o.status==="done");
  const filtered=filter==="all"?[...pending,...active]:filter==="done"?done:orders.filter(o=>o.status===filter);
  ...
  return <div style={{padding:"18px 14px"}}>
    ...
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
      <div>
        <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,fontWeight:600,color:C.deep}}>📦 จัดการ Order</div>
        {pending.length>0&&<div style={{fontSize:12,color:C.alert,marginTop:2,fontWeight:600}}>🔴 รอดำเนินการ {pending.length} รายการ</div>}
      </div>
      <button onClick={()=>setShowForm(true)} style={{...btnSt,width:"auto",marginTop:0,padding:"10px 16px",fontSize:13}}>+ เพิ่ม Order</button>
    </div>

    {/* Filter tabs */}
    <div style={{display:"flex",gap:6,marginBottom:14,overflowX:"auto",paddingBottom:4}}>
      {[{id:"all",label:`ทั้งหมด (${pending.length+active.length})`},{id:"pending",label:`🟡 รอ (${pending.length})`},{id:"clinic",label:`🏥 คลินิก`},{id:"shipping",label:`📦 ส่ง`},{id:"done",label:`✅ เสร็จ (${done.length})`}].map(f=>{
    ...
    <button onClick={load} style={{...secBtnSt,width:"100%",marginBottom:12,textAlign:"center",fontSize:12}}>{loading?"กำลังโหลด...":"🔄 รีเฟรช"}</button>

    {loading
      ?<div style={{textAlign:"center",padding:40,color:C.muted}}>กำลังโหลด...</div>
      :filtered.length===0
        ?<div style={{textAlign:"center",padding:40,color:"#ccc"}}><div style={{fontSize:36,marginBottom:8}}>📭</div><div style={{fontSize:13}}>ไม่มี order ในหมวดนี้</div></div>
        :filtered.map(o=>{
          const st=ORDER_STATUS[o.status]||ORDER_STATUS.pending;
          const isExpand=expandId===o.id;
          const dt=new Date(o.createdAt).toLocaleDateString("th-TH",{day:"numeric",month:"short",year:"2-digit",hour:"2-digit",minute:"2-digit"});
          return <div key={o.id} style={{...cardSt,borderLeft:`4px solid ${st.color}`,paddingLeft:14,marginBottom:10}}>
            <div onClick={()=>setExpandId(isExpand?null:o.id)} style={{cursor:"pointer"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                <div>
                  <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:600,color:C.deep}}>{o.name}</div>
                  <div style={{fontSize:12,color:C.muted,marginTop:2}}>💉 {o.dose} · {o.type==="มาฉีดที่คลินิก"?"🏥 มาคลินิก":"📦 จัดส่ง"}{o.amount?` · ฿${parseInt(o.amount).toLocaleString()}`:""}</div>
                  <div style={{fontSize:11,color:"#bbb",marginTop:2}}>{dt}</div>
                </div>
                <div style={{textAlign:"right"}}>
                  <span style={{fontSize:11,fontWeight:600,padding:"4px 10px",borderRadius:20,background:st.bg,color:st.color}}>{st.emoji} {st.label}</span>
                  <div style={{fontSize:11,color:C.muted,marginTop:4}}>{isExpand?"▲ ซ่อน":"▼ ดูเพิ่ม"}</div>
                </div>
              </div>
            </div>
            {isExpand&&<div style={{marginTop:12,paddingTop:12,borderTop:`1px solid ${C.blush}`}}>
              {o.address&&<div style={{fontSize:12,color:C.muted,marginBottom:8}}>📍 {o.address}</div>}
              {o.note&&<div style={{fontSize:12,color:"#bbb",marginBottom:8}}>📝 {o.note}</div>}
              <div style={{fontSize:11,color:C.muted,marginBottom:10,fontWeight:600}}>เปลี่ยนสถานะ:</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
                {o.status!=="pending"&&<button onClick={()=>updateStatus(o.id,"pending")} style={{padding:"8px",borderRadius:8,border:`1px solid ${C.gold}`,background:"transparent",color:C.gold,fontSize:11,fontFamily:"'DM Sans',sans-serif",cursor:"pointer"}}>🟡 รอดำเนินการ</button>}
                {o.status!=="clinic"&&<button onClick={()=>updateStatus(o.id,"clinic")} style={{padding:"8px",borderRadius:8,border:`1px solid ${C.sage}`,background:"transparent",color:C.sage,fontSize:11,fontFamily:"'DM Sans',sans-serif",cursor:"pointer"}}>🏥 นัดมาคลินิก</button>}
                {o.status!=="shipping"&&<button onClick={()=>updateStatus(o.id,"shipping")} style={{padding:"8px",borderRadius:8,border:`1px solid ${C.rose}`,background:"transparent",color:C.rose,fontSize:11,fontFamily:"'DM Sans',sans-serif",cursor:"pointer"}}>📦 รอจัดส่ง</button>}
                {o.status!=="done"&&<button onClick={()=>updateStatus(o.id,"done",o.dose)} style={{padding:"8px",borderRadius:8,border:"none",background:C.sage,color:"#fff",fontSize:11,fontFamily:"'DM Sans',sans-serif",cursor:"pointer",fontWeight:600}}>✅ เสร็จสิ้น</button>}
              </div>
              <button onClick={()=>{if(window.confirm("ลบ order นี้?"))deleteOrder(o.id);}} style={{width:"100%",padding:"8px",borderRadius:8,border:`1px solid ${C.alert}`,background:"transparent",color:C.alert,fontSize:11,fontFamily:"'DM Sans',sans-serif",cursor:"pointer"}}>🗑️ ลบ Order นี้</button>
            </div>}
          </div>;
        })}
  </>}
