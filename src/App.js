...
const C = {
  ...
  gold: "#C4993E", card: "#FFFFFF", muted: "#9A8478",
  roseDark: "#9A5C3E",
};
...
const SIDE_EFFECTS_OPTIONS = [
  ...
  "ประจำเดือนผิดปกติ","นอนหลับผิดปกติ","ไม่มีผลข้างเคียง",
];
const EXERCISE_OPTIONS = [
  "เดิน 30+ นาที","วิ่ง","ว่ายน้ำ","ขี่จักรยาน",
  "โยคะ/พิลาทิส","เวทเทรนนิ่ง","คาร์ดิโอ","ไม่ได้ออกกำลังกาย",
];
const FOOD_OPTIONS = ["ผลไม้","เครื่องดื่มรสหวาน","โปรตีนชง"];
const DOSE_OPTIONS = ["2.5 mg","5 mg","7.5 mg","10 mg","12.5 mg","15 mg"];
const DOCTOR_PIN = "1234"; // ← เปลี่ยน PIN หมอตรงนี้

// ── Firebase helpers ──────────────────────────────────────
async function fbGet(name) {
  try {
    const snap = await getDoc(doc(db, "patients", name));
    return snap.exists() ? snap.data() : null;
  } catch { return null; }
}
async function fbSet(name, data) {
  try {
    await setDoc(doc(db, "patients", name), data);
    return true;
...
function weeksBetween(a, b) {
  return Math.round((new Date(b) - new Date(a)) / 86400000 / 7);
}
...
const inputSt = {
  ...
  border: `1.5px solid ${C.blush}`, borderRadius: 10,
...
function Toast({ msg, show }) {
  return (
    <div style={{
      ...
      background: C.deep, color: "#fff", padding: "11px 22px",
      borderRadius: 30, fontSize: 13, pointerEvents: "none", zIndex: 999,
...
function CardTitle({ children, color }) {
  return (
    <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 17, fontWeight: 600, color: C.deep, marginBottom: 14, paddingBottom: 10, borderBottom: `1px solid ${C.blush}` }}>
      <span style={{ display: "inline-block", width: 6, height: 6, background: color || C.rose, borderRadius: "50%", marginRight: 8, verticalAlign: "middle" }} />
      {children}
    </div>
  );
}
        ...
        return (
          <button key={o} onClick={() => onChange(on ? selected.filter(x => x !== o) : [...selected, o])}
            style={{
              ...
              border: `1.5px solid ${on ? color : C.blush}`,
              background: on ? color : "transparent",
              color: on ? "#fff" : C.muted,
              fontFamily: "'DM Sans',sans-serif", cursor: "pointer", fontWeight: on ? 500 : 400,
            }}>{o}</button>
        );
      })}
    </div>
...
function RecordDetail({ r }) {
  const has = r.sideEffects?.length > 0 || r.exercise?.length > 0 || r.foodTags?.length > 0 || r.food || r.note;
  if (!has) return null;
  return (
    <div style={{ marginTop: 10, paddingTop: 10, borderTop: `1px solid ${C.blush}`, display: "flex", flexDirection: "column", gap: 4 }}>
      {r.sideEffects?.length > 0 && <div style={{ fontSize: 12, color: C.alert }}>⚡ {r.sideEffects.join(", ")}</div>}
      {r.exercise?.length > 0 && <div style={{ fontSize: 12, color: C.sage }}>🏃 {r.exercise.join(", ")}</div>}
      {r.foodTags?.length > 0 && <div style={{ fontSize: 12, color: C.gold }}>🏷 {r.foodTags.join(", ")}</div>}
    ...
    <div style={{ padding: "18px 14px" }}>
      <div style={{ textAlign: "center", padding: "32px 0 24px" }}>
        ...
        <div style={{ fontSize: 14, color: C.muted, lineHeight: 1.6 }}>
          Astique Clinic — ติดตามผล Mounjaro<br />กรุณากรอกชื่อของคุณเพื่อเริ่มต้น
        </div>
      </div>
      <div style={cardSt}>
        <div style={{ marginBottom: 12 }}>
          <label style={labelSt}>ชื่อของคุณ</label>
          <input style={inputSt} placeholder="เช่น สมหญิง รักสุขภาพ"
            value={val} autoFocus
...
// ── RecordForm ────────────────────────────────────────────
function RecordForm({ patientName, hasHistory, onSave, onViewHistory, saving, showToast }) {
  ...
  function submit() {
    ...
    onSave({ date, weight: w, dose, sideEffects, exercise, foodTags, food, note });
    setDate(todayStr()); setWeight(""); setDose("");
    setSideEffects([]); setExercise([]); setFoodTags([]); setFood(""); setNote("");
  }
  ...
  return (
    <div style={{ padding: "18px 14px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
        <div>
          <div style={{ fontSize: 12, color: C.muted }}>บันทึกโดย</div>
          <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 20, color: C.deep, fontWeight: 600 }}>{patientName}</div>
        </div>
        {hasHistory && <button style={secBtnSt} onClick={onViewHistory}>ดูประวัติ</button>}
      </div>
      <div style={cardSt}>
        <CardTitle>บันทึกวันนี้</CardTitle>
        <div style={{ marginBottom: 12 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
          <div>
            ...
            <input type="number" inputMode="decimal" style={inputSt} placeholder="65.5" step="0.1"
              value={weight} onChange={e => setWeight(e.target.value)} />
          </div>
          <div>
            <label style={labelSt}>ขนาดยา (mg)</label>
        ...
        <div style={{ marginBottom: 14 }}>
          <label style={labelSt}>อาหาร / เครื่องดื่ม</label>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <input style={inputSt} placeholder="เช่น ข้าวต้มไก่เช้า สลัดกลางวัน..."
            value={food} onChange={e => setFood(e.target.value)} />
        </div>
        <div style={{ marginBottom: 4 }}>
          <label style={labelSt}>หมายเหตุ</label>
          <input style={inputSt} placeholder="เช่น รู้สึกดี หิวน้อยลงมาก..."
...
// ── HistoryView ───────────────────────────────────────────
function HistoryView({ patientName, records, onAddNew }) {
  return (
    <div style={{ padding: "18px 14px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
        <div>
      ...
      {records.length === 0
        ? <div style={{ textAlign: "center", padding: 48, color: "#ccc" }}>
            ...
            <div style={{ fontSize: 14 }}>ยังไม่มีข้อมูล</div>
          </div>
            ...
            return (
              <div key={r.date} style={{ ...cardSt, borderLeft: `4px solid ${borderColor}`, paddingLeft: 14, marginBottom: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div>
                    <div style={{ fontSize: 12, color: C.rose, fontWeight: 500, marginBottom: 2 }}>{fmtDate(r.date)}</div>
                    ...
                    <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>💉 {r.dose}</div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    {status === "first" && <Badge color={C.gold}>⭐ เริ่มต้น</Badge>}
                    {status === "good" && <Badge color={C.sage}>✓ ดีเยี่ยม −{diff.toFixed(1)} กก.</Badge>}
                    {status === "low" && <Badge color={C.alert}>⚠ ลดน้อย −{diff.toFixed(1)} กก.</Badge>}
                    {status === "gain" && <Badge color={C.alert}>⚠ ขึ้น +{Math.abs(diff).toFixed(1)} กก.</Badge>}
                  </div>
                </div>
                <RecordDetail r={r} />
              </div>
...
function ChartView({ patientName, records }) {
  const chartData = records.map((r, i) => ({ date: fmtDate(r.date), weight: r.weight, status: getStatus(records, i) }));
      ...
      <div style={cardSt}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 16 }}>
          {[
            ...
            { val: records.length >= 2 ? weeksBetween(records[0].date, records[records.length - 1].date) + " สป." : "—", label: "ช่วงเวลา" },
          ].map(st => (
        ...
        <div style={{ marginTop: 12, display: "flex", gap: 14, justifyContent: "center" }}>
          {[{ color: C.sage, label: "ลด ≥ 0.5 กก. ✓" }, { color: C.alert, label: "ต้องติดตาม ⚠" }, { color: C.gold, label: "เริ่มต้น ⭐" }].map(l => (
            ...
            return (
              <div key={r.date} style={{ background: "rgba(201,59,59,0.05)", border: "1.5px solid rgba(201,59,59,0.2)", borderRadius: 10, padding: "12px 14px", marginBottom: 8, display: "flex", gap: 10 }}>
                ...
                <div style={{ fontSize: 13, color: C.alert, lineHeight: 1.5 }}>
                  <strong>{fmtDate(r.date)}</strong> — น้ำหนัก{diff >= 0 ? `ลดน้อยเพียง −${diff.toFixed(1)}` : `ขึ้น +${Math.abs(diff).toFixed(1)}`} กก.<br />
                  <span style={{ fontSize: 11, color: C.muted }}>ขนาดยา: {r.dose} · แนะนำแจ้งหมอทิพย์</span>
                </div>
              </div>
            );
          })}
...
function DoctorView() {
  ...
  if (selected) {
    ...
    const cdata = recs.map((r, i) => ({ date: fmtDate(r.date), weight: r.weight, status: getStatus(recs, i) }));
    return (
      <div style={{ padding: "18px 14px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
          {[
            ...
            { val: recs.length >= 2 ? weeksBetween(recs[0].date, recs[recs.length - 1].date) + " สป." : "—", label: "ช่วงเวลา" },
          ].map(st => (
        ...
        {[...recs].reverse().map(r => {
          ...
          return (
            <div key={r.date} style={{ ...cardSt, borderLeft: `4px solid ${borderColor}`, paddingLeft: 14, marginBottom: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div>
                  <div style={{ fontSize: 12, color: C.rose, fontWeight: 500 }}>{fmtDate(r.date)}</div>
                  ...
                  <div style={{ fontSize: 12, color: C.muted }}>💉 {r.dose}</div>
                </div>
                <div>
                  {status === "first" && <Badge color={C.gold}>⭐ เริ่มต้น</Badge>}
                  {status === "good" && <Badge color={C.sage}>✓ −{diff.toFixed(1)} กก.</Badge>}
                  {status === "low" && <Badge color={C.alert}>⚠ −{diff.toFixed(1)} กก.</Badge>}
                  {status === "gain" && <Badge color={C.alert}>⚠ +{Math.abs(diff).toFixed(1)} กก.</Badge>}
                </div>
              </div>
              <RecordDetail r={r} />
            </div>
          ...
          : allPatients.map(p => {
              ...
              return (
                <div key={p.name} onClick={() => setSelected(p)} style={{ ...cardSt, cursor: "pointer", borderLeft: `4px solid ${alert ? C.alert : recs.length <= 1 ? C.gold : C.sage}`, paddingLeft: 14, marginBottom: 10 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div>
                      ...
                      <div style={{ fontSize: 12, color: C.muted, marginTop: 3 }}>
                        {recs.length} บันทึก{last && ` · ล่าสุด ${fmtDate(last.date)}`}{last?.dose && ` · 💉 ${last.dose}`}
                      </div>
                      {diff !== null && (
                        <div style={{ fontSize: 12, color: diff >= 0.5 ? C.sage : C.alert, marginTop: 2, fontWeight: 500 }}>
                  ...
                  {last?.sideEffects?.length > 0 && <div style={{ marginTop: 6, fontSize: 11, color: C.alert }}>⚡ {last.sideEffects.join(", ")}</div>}
                  {last?.exercise?.length > 0 && <div style={{ fontSize: 11, color: C.sage }}>🏃 {last.exercise.join(", ")}</div>}
                </div>
              );
            })}
    </div>
  ...
  return (
    <div style={{ padding: "40px 32px", display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      <div style={{ display: "flex", gap: 16, marginBottom: 36, animation: shake ? "shake .4s" : "none" }}>
        {[0,1,2,3].map(i => (
          <div key={i} style={{ width: 14, height: 14, borderRadius: "50%", background: i < pin.length ? C.rose : "transparent", border: `2px solid ${i < pin.length ? C.rose : C.blush}`, transition: "all .15s" }} />
        ))}
      </div>
...
export default function App() {
  ...
  async function handleSave(record) {
    if (records.find(r => r.date === record.date)) return showToast("มีข้อมูลวันนี้แล้ว");
    const newRecords = [...records, record].sort((a, b) => a.date.localeCompare(b.date));
    setSaving(true);
    ...
    <>
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input:focus, select:focus { border-color: #C07A58 !important; outline: none; }
      ...
      <div style={{ fontFamily: "'DM Sans',sans-serif", background: C.cream, minHeight: "100vh", maxWidth: 480, margin: "0 auto" }}>
        ...
        <div style={{ background: C.deep, padding: "18px 20px 14px", display: "flex", alignItems: "center", gap: 13, position: "sticky", top: 0, zIndex: 100 }}>
          ...
          <button
            onClick={() => { if (mode === "doctor") { setMode("patient"); setDoctorUnlocked(false); setTab("patient"); } else setMode("doctor"); }}
        ...
        {mode === "patient" && <>
          ...
          {tab === "patient" && phase === "history" && <HistoryView patientName={patientName} records={records} onAddNew={() => setPhase("form")} />}
        ...
        {mode === "doctor" && (doctorUnlocked
          ? <>
              <div style={{ background: C.deep, padding: "0 16px 14px" }}>
                <button style={tabBtnSt(true)}>👩‍⚕️ คนไข้ทั้งหมด</button>
              </div>
              <DoctorView />
            </>
